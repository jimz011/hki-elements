...
Robot vacuum control and status. Shared as-is from my own Home Assistant dashboard for reference/inspiration — entity IDs, sensors, and automations are specific to my setup and won't work unmodified in yours.
  ...
  - type: sections
    ...
    sections:
      - type: grid
        cards:
          - type: conditional
            conditions:
              - entity: input_boolean.vacuum_selector
                state: 'off'
            card:
              type: vertical-stack
              cards:
                ...
                - type: custom:xiaomi-vacuum-map-card
                  entity: vacuum.valetudo_roborock_q7_max
                  map_locked: true
                  two_finger_pan: true
                  map_source:
                    camera: camera.valetudo_q7_max_freshzigzagskunk_camera
                  calibration_source:
                    camera: camera.valetudo_q7_max_freshzigzagskunk_camera
            ...
            card:
              ...
              cards:
                ...
                - type: custom:xiaomi-vacuum-map-card
                  ...
                  two_finger_pan: true
                  map_source:
                    camera: camera.valetudo_s5_gentlepossiblescorpion_camera
                  calibration_source:
                    camera: camera.valetudo_s5_gentlepossiblescorpion_camera
                  vacuum_platform: Hypfer/Valetudo
            grid_options:
    ...
    header:
      card:
        ...
        title: Roborock
        subtitle: Vacuum Control
        background: >-
          https://global.roborock.com/cdn/shop/t/26/assets/reactff_section-first-bg.jpg?v=180405919845824700491645774642
        min_height: 215
        max_height: 270
        title_offset_y: 65
    ...
    badges:
      - type: custom:mushroom-template-badge
        card_mod:
          style: |
            :host {
              {% if is_state('input_boolean.vacuum_selector', 'off') %}
        ...
        tap_action:
          ...
          target:
            entity_id: input_boolean.vacuum_selector
        visibility:
          - condition: screen
            media_query: '(min-width: 0px) and (max-width: 1023px)'
      - type: custom:mushroom-template-badge
        card_mod:
          style: |
            :host {
              {% if is_state('input_boolean.vacuum_selector', 'on') %}
                --ha-card-background: white;
                color: var(--accent-color);
                --primary-text-color: black;
              {% endif %}
