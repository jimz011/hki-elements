  ...
  - type: sections
    ...
    header:
      card:
        ...
        title: Attic Front
        subtitle: |
          {% if is_state('media_player.zolder_voor_speaker', 'playing') %}
            Music: {{ state_attr('media_player.zolder_voor_speaker', 'media_artist') }} - {{ state_attr('media_player.zolder_voor_speaker', 'media_title') }}
          {% else %}
            No Media is Playing
          {% endif %}
        background: /local/images/rooms/attic_front.png
        min_height: 215
        max_height: 270
        title_offset_y: 65
