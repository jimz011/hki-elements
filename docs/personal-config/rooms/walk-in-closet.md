  ...
  - type: sections
    ...
    header:
      card:
        ...
        title: Walk-in-Closet
        subtitle: |
          {% if is_state('media_player.walk_in_closet_speaker', 'playing') %}
            Music: {{ state_attr('media_player.walk_in_closet_speaker', 'media_artist') }} - {{ state_attr('media_player.walk_in_closet_speaker', 'media_title') }}
          {% else %}
            No Media is Playing
          {% endif %}
        background: /local/images/rooms/wic.png
        min_height: 215
        max_height: 270
        title_offset_y: 65
